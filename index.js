...
function renderNews() {
  ...
  let html = newsDataRender
    .map((item) => {
      // 替换HTML模板中的占位符{{title}}和{{content}}为新闻数据的对应值
      let result = tpl
...
async function run() {
  ...
  // 为skipInput元素添加按键事件监听器
  skipInput.addEventListener("keypress", (event) => {
    // 判断按下的键是否为Enter键
    if (event.key === "Enter") {
      // 调用changePageClass函数，传入skipInput.value作为参数，用于改变分页状态
      changePageClass(skipInput.value);
      // 将跳转页码赋值给变量p
      p = skipInput.value;
      // 调用renderNews函数，重新渲染新闻列表
      renderNews();
    }
  });
}
